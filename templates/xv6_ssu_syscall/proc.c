...
int do_fork(enum Sched_State type, int nice)
{
	int i, pid;
	struct proc *np;
	struct proc *curproc = myproc();
...
fork_rt(int nice)
{
	// If calling fork_rt(nice), running do_fork(RT, nice)
	return do_fork(RT, nice);
}
...
{
  struct proc *p;
  static struct proc *last_rt_p = &ptable.proc[NPROC - 1];	// last point for Real-Time Process
  static struct proc *last_rr_p = &ptable.proc[NPROC - 1];	// last point for Round-Robin Process
  struct cpu *c = mycpu();
  c->proc = 0;
...
dispatch_rt:
    last_rt_p = p; // Update last RT Point
    goto execute;

dispatch_rr:
    last_rr_p = p; // Update last RR Point
    goto execute;
...
execute:	   // Execution Part
    ...
    c->proc = 0;
    // After completing the run, go back to the loop first.
    // And check again RT unconditionally.
    release(&ptable.lock);
  }
}
